...
description: "将 Screen Studio 工程或普通视频及其配套字幕整理为本地公众号 Markdown 文章，截取并核对真实画面，默认搭配 oil-tone。用户要求视频转文章、公众号图文稿或补视频文章配图时使用。不因仅提供视频或字幕而触发；不承担字幕转录、工程剪辑或直接发布，分别交给字幕、剪辑或发布工具。"
---
...
- 总时长相近也不能证明时间轴相同。先在前、中、后和拟用配图位置核对导出视频与工程映射画面。
- 不一致时，优先找到导出时的工程备份或可靠的剪辑映射；不使用单一总差值推导全部时间点。无法证明对应关系时改用导出视频截帧，并说明无法保证纯屏幕轨画面。
...
优先通过官方文档、只读网页抓取或已提供的原始资料取得正文；只有用户明确授权浏览器自动化时才使用对应浏览器工具。核对内容后把原文的命令或配置以代码块形式插进文章对应段落，例如一键脚本、PowerShell 命令、安装语句。补充的内容必须是页面上真实存在的原文，不允许凭记忆或猜测补写命令。文章仍按视频主线组织，只补充视频实际讲到的那几处，不要把整篇文档搬进文章。
...
- 结尾说完就停，不加感悟和升华；
- 写完运行 `python3 "<实际 oil-tone 安装目录>/scripts/tone_lint.py" <成稿路径>`，结合上下文检查提示；没有提示也要通读全文，不机械替换原本自然的表达。
